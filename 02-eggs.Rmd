...
jpi_sacpas <- read_delim("https://www.cbr.washington.edu/sacramento/data/php/rpt/jpe.php?sc=1&year=all")%>%
...
tdm <- read_csv("data_raw/ETF_TDM_2002_2023.csv") %>%
  clean_names() %>%
  filter(brood_year <= report_year) %>%
  mutate(unexplained_mortality = 100-etf_survival-tdm_noaa_percent) %>%
  filter(brood_year <= report_year) %>%
  mutate(BY = brood_year) %>%
  mutate(brood_year = factor(brood_year))%>%
  mutate(color = case_when(sac_val_year_type == "C" ~ "#D55E00",
                           sac_val_year_type == "D" ~ "#E69F00",
                           sac_val_year_type == "AN" ~ "#009E73",
                           sac_val_year_type == "BN" ~  "black",
                           sac_val_year_type == "W" ~ "#0072B2")) %>%
  mutate(brood_year_type = paste0(brood_year, " (", sac_val_year_type, ")" ))

tdm_long <- tdm %>%
  select(brood_year_type, color,
...
tenyear_etf_avg = mean(tenyear_etf$ETF_Survival)
twentyyear_etf = jpi %>% filter(BY > report_year-20 & BY<= report_year)
twentyyear_etf_avg = mean(twentyyear_etf$ETF_Survival)

dir_etf1 <- ifelse(current.ETF<tenyear_etf_avg, "below", "above")
dir_etf2 <- ifelse(current.ETF<twentyyear_etf_avg, "below", "above")
```
...
-   **Fry-equivalents at RBDD (JPE Letter):** `r scales::comma(as.numeric(jpi_sacpas$value[jpi_sacpas$brood_year == report_year]))`. 
    -   **10-year average:** `r scales::comma(mean(as.numeric(jpi_tenyear$value), na.rm = TRUE))` including 2011, 2014-2017, 2021
-   **Egg to Fry Survival (ETF Survival; JPI-based):** ETF Survival in `r report_year` was `r dir_etf1` the 10-year average ETF and `r dir_etf2` the 20-year average ETF (Figure \@ref(fig:ETFSurv-fig)). 
    - **10-year average:** `r round(tenyear_etf_avg,1)`%
    - **20-year average:** `r round(twentyyear_etf_avg,1)`%
...
The SacPAS Fish Model was developed by the University of Washington Columbia Basin Research center. The model, along with additional method and usage details, are available on: ["the SacPAS website"](https://www.cbr.washington.edu/sacramento/fishmodel/).

The user-selected inputs to the model include:

- `r report_year` carcass survey, temperature, and flow data
...
(plot_tdm <- ggplot(tdm_long, aes(brood_year_type, Percent, fill = Fate)) + 
