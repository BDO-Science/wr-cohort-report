...
```{r historicalwtemp-fig, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = paste("Daily Water Temperature at Sacramento River at Clear Creek (CCR) and Sacramento River at Balls Ferry Bridge (BSF) in", report_year, "and over the 10-year average"), fig.width = 8, fig.height = 6}

wtemp_bsf_ccr_plot_data <- readRDS(paste0("data_raw/wtemp_CCR_BSF_plot_data_", year(start), "-", year(end), ".rds"))

ggplotly(ggplot(wtemp_bsf_ccr_plot_data) + geom_line(aes(date, WaterTemp, color = plot_year, linetype = plot_year)) +
...
* Sacramento River system-wide total adult escapement: `r esc_all_summary$Escapement[esc_all_summary$Year == report_year]`
...
* Total mainstem in-river spawner estimate (Killam): `r esc_sacriver$Escapement[esc_sacriver$Year == report_year]`, 90% Confidence Interval: []
...
```{r carcass-tab,tab.cap = "Carcass Data Summary. Fork length cutoffs are 610 mm for Females and 680 mm for Males based on Killam 2021"}
...
```{r carcassFL-fig,fig.cap = "Carcass Data Fork Length Distribution. Plots are separated by Males and Females."}
...
```{r spawntiming-fig, fig.cap = paste0("Spawn Timing in", report_year)}
ggplot(spawn_timing_hist)+
  geom_line(aes(datestart, percent, color = year_fac, linetype = current)) +
...
ggplot(peak_spawning) + geom_col(aes(year_fac, week), fill = "steelblue4") +
...
carcass_hist_table <- carcass_hist_avg %>%
  ...
  select(Section_Name, everything())
...
#### Redd Distribution
```{r, redddistrib-fig, fig.cap = "Distribution of Winter Run Redds"}
