...
shasta_data <- readRDS(paste0("data_raw/storage_sha_plot_data_", year(start), "-", year(end), ".rds")) %>%
   ...
   mutate(Month = month(doy, label = TRUE, abbr = TRUE)) %>%
  mutate(Storage_TAF = round(Storage/1000000,2))
  

ggplot(shasta_data) + 
  geom_line(aes(doy, Storage_TAF, color = plot_year, linetype = plot_year)) + 
...
- In `r report_year`, storage was consistently below the 10-year average (Figure \@ref(fig:SHAstor-fig)), with peak storage at __`r max_storage_sha`__ TAF in **`r max_storage_sha_month`**. 

#### Flow Conditions on the Sacramento River at Keswick (KWK) and Sacramento River at Bend Bridge (BND)
...
ggplot(flow_kwk_bnd) + 
  geom_line(aes(doy, flow, color = plot_year, linetype = plot_year)) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b %d") +
  scale_color_manual(values = c("black", "steelblue")) +
  scale_linetype_manual(values = c("dashed", "solid")) +
...
-   **Keswick:** Peak flows were __`r max_flow_kwk`__ cfs and occurred in **`r max_flow_kwk_month[1]`**. The highest mean flows were **`r max_mean_flow_kwk`** cfs and occurred in **`r max_mean_flow_kwk_month`**.
...
wtemp_bsf_ccr_plot_data <- readRDS(paste0("data_raw/temp_CCR_BSF_plot_data", year(start), "-", year(end), ".rds"))
wtemp_bsf_ccr_monthly <- readRDS(paste0("data_raw/temp_CCR_BSF_monthly", year(start), "-", year(end), ".rds"))


ggplot(wtemp_bsf_ccr_plot_data) + geom_line(aes(doy, WaterTemp, color = plot_year, linetype = plot_year)) +
...
-   **Balls Ferry:** Maximum water temperature was __`r max_wtemp_bsf`__ degrees F and occurred in **`r max_wtemp_bsf_month[1]`**. The highest mean water temperature was **`r max_mean_wtemp_bsf`** degrees F and occurred in **`r max_mean_wtemp_bsf_month`**.
...
```{r Turb-fig, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Dissolved Oxygen at Keswick Dam (KWK) and Sacramento River upstream from Confluence with Clear Creek (CCR)."}

do_kwk_ccr_plot_data <- readRDS(paste0("data_raw/do_KWK_CCR_plot_data_", year(start), "-", year(end), ".rds"))

ggplot(do_kwk_ccr_plot_data) + geom_line(aes(doy, DO, color = plot_year, linetype = plot_year)) +
...
```{r echo = FALSE}
monthly_do <- do_kwk_ccr_monthly %>% filter(station == "KWK")
...
-   **Keswick:** Minimum dissolved oxygen was __`r min_do_kwk`__ mg/L and occurred in **`r min_do_kwk_month[1]`**. The lowest mean dissolved oxygen was **`r min_mean_do_kwk`** mg/L and occurred in **`r min_mean_do_kwk_month`**.
