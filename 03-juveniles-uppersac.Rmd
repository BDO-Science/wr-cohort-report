...
```{r KWKBNDflowjuv-fig, fig.cap = paste("Daily Flows (cfs) at Sacramento River at Keswick (KWK) and Sacramento River at Bend Bridge (BND) in July through September",report_year, "and over the 10-year average"), fig.width = 8, fig.height = 6}
...
```{r wtempBND-fig,  fig.cap = paste("Daily Water Temperature (degF) at Sacramento River at Bend (BND) in",report_year,"and 10-year average between July and December.") , fig.width = 8, fig.height = 4.5}
...
```{r doBNDjuv-fig, fig.cap = paste("Daily Dissolved Oxygen (mg/L) at Sacramento River at Bend Bridge (BND) in July through December",report_year,"and 10-year average."), fig.width = 8, fig.height = 4.5}
...
### Turbidity
```{r TurbBNDRDB-fig, fig.cap = paste("Daily Turbidity at Sacramento River at Bend Bridge (BND) and Red Bluff Diversion Dam (RDB) in",report_year, "and 10-year average between July and December. Turbidity data have not undergone QC, other than values filtered to less than 300 NTU."), fig.width = 8, fig.height = 5}
...
```{r, echo = FALSE}
# Figure of Estimated Percent Fry Passage
```

```{r JPI-fig, fig.cap = paste("Annual JPI from 2002 to",report_year)}
(plot_jpi <- ggplot(jpi) + 
...
# Raw Fork Lengths
fl_raw_rbdd <- read.csv("data_raw/raw_wr_rbdd.csv") %>%
  mutate(date = ymd(start_date))

fl_raw_rbdd_avg <- fl_raw_rbdd %>%
  mutate(year = year(date),
         month = month(date),
         day = day(date),
         by = if_else(month %in% c(1,2,3,4,5,6), year-1, year)) %>%
  filter(by>report_year-10 & by <= report_year) %>%
  mutate(year_type = if_else(by == report_year, as.character(report_year), "10-year average")) %>%
  mutate(date2 = if_else(month %in% c(1,2,3,4,5,6), ymd(paste0(report_year+1, "-", month, "-", day)),ymd(paste0(report_year, "-", month, "-", day)))) %>%
  group_by(year_type, date2) %>%
  summarize(fork_length = mean(fork_length,na.rm = TRUE))
  
fl_raw_rbdd_current <- fl_raw_rbdd %>%
  filter(date >= ymd(paste0(report_year, "07-01"))& date<ymd(paste0(report_year+1,"06-01" ))) %>%
  mutate(month = month(date)) %>%
  mutate(period = case_when(month %in% c(7,8) ~ "early",
                            date >= rbdd_by$x25_percent_passage_date & date <rbdd_by$x75_percent_passage_date ~ "middle 50%",
                            month %in% c(3,4,5) ~ "late"))

fl_raw_current_avg <- fl_raw_rbdd_current %>%
  group_by(period) %>%
  summarize(min = round(min(fork_length)),
            max = round(max(fork_length)),
            mean = round(mean(fork_length)))

p_fry <- fl_raw_rbdd_current %>%
  mutate(fry = if_else(fork_length<46, "yes", "no")) %>%
  group_by(fry) %>%
  summarize(total = sum(count))

percent_fry <- round(p_fry$total[2]/sum(p_fry$total) *100)

fl_avg_table <- fl_raw_rbdd_avg %>%
  group_by(year_type) %>%
  summarize(min = min(fork_length),
            max = max(fork_length))
```


- **Average daily fork length** 
    - Early migration (July to August): `r fl_raw_current_avg$min[1]` to `r fl_raw_current_avg$max[1]` mm
    - Middle 50% of cumulative (`r format(rbdd_by$x25_percent_passage_date, "%B %d")` to `r format(rbdd_by$x75_percent_passage_date, "%B %d")`): `r fl_raw_current_avg$min[2]` to `r fl_raw_current_avg$max[2]` mm
    - Late migration: `r fl_raw_current_avg$min[2]` to `r fl_raw_current_avg$max[2]` mm
- `r percent_fry` % of fish migrated past RBDD as fry (<46 mm FL)
- Average daily fork length in `r report_year` was `r fl_avg_table$min[2]` to `r fl_avg_table$max[2]`mm compared with the 10-year average daily fork length (`r fl_avg_table$min[1]` to `r fl_avg_table$max[1]` mm)


```{r rbdd-fl-fig, fig.cap = paste0("Fork Length Distribution in ",report_year, " and 10-year average at RBDD RST. The yellow line at 45 mm indicates the length transition from fry to smolt.") }
ggplotly(ggplot(fl_rbdd) + 
 
  geom_point(data = fl_raw_rbdd_avg, aes(date2, fork_length,color = year_type), size = 1)+
   geom_errorbar(data = fl_rbdd, aes(date2, ymin = length_min, ymax = length_max), alpha = 0.6) +
    geom_hline(yintercept = 45, color = "gold2") +
...
```{r rbdd-flhist-fig, fig.cap = "Fork Length Distribution over Time, RBDD RST" }
ggplot(fl_raw_rbdd) + 
  geom_histogram(aes(fork_length), color = "black", fill = "forestgreen", binwidth = 5) +
